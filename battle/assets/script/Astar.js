function Astar (mapBlockLayer, mapWidth, mapHeight) {
    ...
    this.search = function (start, end) {
        ...
        var resultList = search(start, end);
    ...
    function search(start, end, unitBlockList) {
        blocks = [];
        ...
        while (openList.length > 0) {
            ...
            if (node.y - 1 >= 0) {
                checkPath(cc.p(node.x, node.y - 1), node, end, moveCost);
            }
            if (node.y + 1 < height) {
                checkPath(cc.p(node.x, node.y + 1), node, end, moveCost);
            }
            if (node.x - 1 >= 0) {
                checkPath(cc.p(node.x - 1, node.y), node, end, moveCost);
            }
            if (node.x + 1 < width) {
                checkPath(cc.p(node.x + 1, node.y), node, end, moveCost);
            }
    ...
    function checkPath (pos, parent, end, cost) {
        var node = {
            x: pos.x,
            y: pos.y,
            parent: parent,
        ...
        if (pos.x >= this.mapWidth || pos.y >= this.mapHeight || pos.x < 0 || pos.y < 0) {
            return false;
        }
        if(!checkBlock(pos)) {
            closeList.push(node);
